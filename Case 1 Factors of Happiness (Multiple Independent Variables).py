...
for row in csvreader:
    ...
    # 1. Economy (GDP Per Capita relative to the United States)
    # 2. Faimly Score
    # 3. Life Expectancy
    # 4. Freedom
    # 5. Government Trust
    # 6. Community Generosity
    # 7. Dystopia Residual
    X.append([float(row[6]), float(row[7]), float(row[8]), float(row[9]), float(row[10]), float(row[11]), float(row[12])])
...
R_Squared = 1 - (SSR / TSS)
Adjusted_R_Squared = 1 - (1 - R_Squared**2) * (157 - 1) / (157 - 7 - 1)
